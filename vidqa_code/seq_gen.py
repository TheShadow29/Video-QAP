...
class SequenceGenerator(object):
    ...
    def _generate(self, model, sample, prefix_tokens=None, bos_token=None, **kwargs):
        ...
        encoder_input = {k: v for k, v in sample.items() if "prev_out" not in k}
        assert "encoder_key" in kwargs, "Need what to use for src_tokens"
        src_tokens = encoder_input[kwargs["encoder_key"]].squeeze(1)
        src_lengths = encoder_input[kwargs["encoder_key_len"]]

        bsz = src_tokens.size(0)

        beam_size = self.beam_size

        max_len = min(int(self.max_len_b), model.max_decoder_positions() - 1,)
...
class EnsembleModel(torch.nn.Module):
    ...
    def __init__(self, models):
        super().__init__()
        if isinstance(models[0], torch.nn.parallel.DistributedDataParallel):
            assert all(
                [
            ...
            self.dist_model = True
            model_lst = [m.module for m in models]

        else:
            self.dist_model = False
            model_lst = models

        self.models = torch.nn.ModuleList(model_lst)

        self.incremental_states = None

        if all(isinstance(m.decoder, FairseqIncrementalDecoder) for m in model_lst):
            self.incremental_states = {m: {} for m in model_lst}
    ...
    def forward_encoder(self, encoder_input):
        ...
        out = [model.forward_encoder(encoder_input) for model in self.models]
        return out
